...
class MainActivity : AppCompatActivity() {
    ...
    fun task16(store: Map<String, UInt>, asset: Map<String, UInt>): UInt {
        var minPossibleItems = UInt.MAX_VALUE

        for ((item, quantity) in asset) {
            val storeQuantity = store[item] ?: 0u // Sprawdzenie, czy element jest dostępny w magazynie
            if (storeQuantity < quantity) {
                return 0u // Nie można zbudować żadnego egzemplarza, gdy brakuje jednego z elementów
            }
            val possibleItems = storeQuantity / quantity // Obliczenie możliwej liczby egzemplarzy na podstawie aktualnego stanu magazynu

            if (possibleItems < minPossibleItems) {
                minPossibleItems = possibleItems
            }
        }

        return minPossibleItems
    }
}